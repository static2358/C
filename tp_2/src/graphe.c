...
// Parcours en profondeur (DFS) - fonction auxiliaire récursive
void dfsRecursif(Graphe *g, int sommet, int *visite, int *ordre, int *compteur) {
    visite[sommet] = 1;
    (*compteur)++;
    ordre[sommet] = *compteur;
    printf("Sommet %d visite (ordre %d)\n", sommet, *compteur);
    
    for (int i = 0; i < g->nbSommets; i++) {
        if (g->adjacence[sommet][i] == 1 && !visite[i]) {
            dfsRecursif(g, i, visite, ordre, compteur);
        }
    }
}
...
void parcoursEnProfondeur(Graphe *g, int sommetDepart) {
    if (!g || sommetDepart < 0 || sommetDepart >= g->nbSommets) {
        fprintf(stderr, "Erreur : sommet de depart invalide\n");
        return;
    }
    
    int *visite = (int *)calloc(g->nbSommets, sizeof(int));
    int *ordre = (int *)calloc(g->nbSommets, sizeof(int));
    int compteur = 0;
    
    printf("\n=== PARCOURS EN PROFONDEUR (DFS) depuis sommet %d ===\n", sommetDepart);
    dfsRecursif(g, sommetDepart, visite, ordre, &compteur);
    
    printf("\nOrdre de marquage:\n");
    for (int i = 0; i < g->nbSommets; i++) {
        if (ordre[i] > 0) {
            printf("Sommet %d : ordre %d\n", i, ordre[i]);
        } else {
            printf("Sommet %d : non visite\n", i);
        }
    }
...
void parcoursEnLargeur(Graphe *g, int sommetDepart) {
    ...
    int *visite = (int *)calloc(g->nbSommets, sizeof(int));
    int *ordre = (int *)calloc(g->nbSommets, sizeof(int));
    int *file = (int *)malloc(g->nbSommets * sizeof(int));
    int debut = 0, fin = 0;
    int compteur = 0;
    
    printf("\n=== PARCOURS EN LARGEUR (BFS) depuis sommet %d ===\n", sommetDepart);
    
    file[fin++] = sommetDepart;
    visite[sommetDepart] = 1;
    compteur++;
    ordre[sommetDepart] = compteur;
    printf("Sommet %d visite (ordre %d)\n", sommetDepart, compteur);
